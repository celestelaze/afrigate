    ...
    <div className="overflow-x-hidden">
            ...
            <div className="animate-fade-up animate-delay-200">
              <div className="glass rounded-3xl p-6 mb-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {allCountries.map((c) => (
                    <div key={c.code} className="flex items-center gap-3 bg-white/10 rounded-xl px-3 py-2.5 hover:bg-white/15 transition-colors">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[{val:10000,suffix:'+',label:'Transactions'},{val:7,suffix:'',label:'Pays couverts'},{val:98,suffix:'%',label:'Satisfaction'},{val:24,suffix:'h/7j',label:'Support humain'}].map(s => (
      ...
      <section id="how-it-works" className="py-24 bg-white">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8 relative">
            {[
              {num:'01',icon:'🔢',title:'Simulez votre transfert',desc:"Choisissez les pays, entrez le montant, et voyez instantanément le calcul des frais et le montant reçu en FCFA ou MAD."},
              {num:'02',icon:'💬',title:'Confirmez sur WhatsApp',desc:"Un message pré-rempli avec tous les détails s'envoie automatiquement à notre équipe via WhatsApp Business."},
              ...
              <div key={i} className="relative text-center group">
                <div className="w-16 h-16 rounded-2xl bg-navy/5 flex items-center justify-center text-3xl mx-auto mb-5 group-hover:bg-gold-DEFAULT/10 transition-colors">{s.icon}</div>
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-2 w-7 h-7 rounded-full bg-gold-DEFAULT text-navy text-xs font-extrabold flex items-center justify-center">{i+1}</div>
                {i < 2 && <div className="hidden md:block absolute top-8 left-[calc(50%+40px)] right-[calc(-50%+40px)] h-0.5 bg-gradient-to-r from-gold-DEFAULT/60 to-navy/20" />}
          ...
          <div className="text-center mt-12">
            <Link to="/transfer" className="inline-flex items-center gap-2 px-8 py-4 rounded-2xl bg-navy text-white font-bold hover:bg-navy-light transition-colors shadow-lg shadow-navy/20">
      ...
      <section className="py-20 bg-navy/5">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <p className="text-gray-500 mb-8">Voyez en temps réel combien votre bénéficiaire recevra.</p>
          <QuickSimulator />
        </div>
      </section>
            ...
            {TESTIMONIALS.map((t,i) => (
              <div key={i} className="glass rounded-2xl p-6 card-hover">
                <div className="flex gap-1 mb-4">
                  {Array.from({length:t.stars}).map((_,j) => <Star key={j} size={14} className="text-gold-DEFAULT fill-gold-DEFAULT" />)}
                </div>
                <p className="text-blue-100 text-sm leading-relaxed mb-5 italic">"{t.text}"</p>
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gold-DEFAULT/20 flex items-center justify-center text-gold-DEFAULT font-bold text-sm">{t.initials}</div>
                  <div>
      ...
      <section className="py-16 bg-gray-50">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-wrap justify-center items-center gap-6">
            {ALL_PARTNERS.map(p => (
              <div key={p.name}
                className="h-16 px-5 rounded-2xl flex items-center justify-center shadow-sm border border-gray-100"
      ...
      <section className="py-20 bg-mesh">
        <div className="max-w-3xl mx-auto px-4 text-center">
          <h2 className="font-display text-3xl lg:text-4xl font-extrabold text-white mb-4">Prêt à envoyer de l'argent ?</h2>
          ...
          <Link to="/transfer"
            className="inline-flex items-center gap-2 px-9 py-4 rounded-2xl bg-gold-DEFAULT text-navy font-bold text-lg hover:bg-gold-light transition-all shadow-xl shadow-gold-DEFAULT/30">
...
function QuickSimulator() {
  ...
  const fees = num ? Math.round(num * 0.1 * 100) / 100 : 0
  const total = num ? Math.round((num + fees) * 100) / 100 : 0
  const received = num
    ? isMad ? Math.round(num * rate) : Math.round((num / rate) * 100) / 100
    : 0
  ...
  return (
    <div className="bg-white rounded-3xl shadow-xl p-8 text-left">
      ...
      <label className="block text-sm font-semibold text-navy mb-2">
        Montant à envoyer ({isMad ? 'MAD — Dirham marocain' : 'FCFA — Franc CFA'})
      </label>
      <input type="number" value={amount} onChange={e => setAmount(e.target.value)}
        placeholder={isMad ? 'ex: 500 MAD' : 'ex: 25 000 FCFA'}
      ...
      {/* Live result */}
      {num > 0 && (
        <div className="bg-navy rounded-2xl p-5 space-y-3 mb-6">
          <div className="flex justify-between">
            <span className="text-blue-200 text-sm">Montant envoyé</span>
            <span className="text-white font-bold">{num.toLocaleString()} {isMad ? 'MAD' : 'FCFA'}</span>
          </div>
          <div className="flex justify-between">
            <span className="text-blue-200 text-sm">Frais de service (10%)</span>
            <span className="text-white font-bold">+ {fees.toLocaleString()} {isMad ? 'MAD' : 'FCFA'}</span>
          </div>
          <div className="flex justify-between">
          ...
          <div className="pt-3 border-t border-white/10 text-center">
            <span className="text-blue-200 text-xs uppercase tracking-widest font-semibold block mb-2">Le bénéficiaire reçoit</span>
            <span className="text-gold-DEFAULT font-extrabold text-4xl block leading-tight">
              {received.toLocaleString()}
            </span>
            <span className="text-gold-DEFAULT font-bold text-xl block mt-1">
              {isMad ? 'FCFA' : 'MAD'}
            </span>
          </div>
        </div>
      ...
      <Link to="/transfer"
        className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl bg-gold-DEFAULT text-navy font-bold hover:bg-gold-light transition-colors">
        Effectuer ce transfert <ChevronRight size={18} />
      </Link>
    </div>
  )
}
